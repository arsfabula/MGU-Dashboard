...
- **Enregistrement FIT** : puissance cycliste, puissance moteur et cadence via les champs natifs Garmin (moyennes, tours, minima reconnus par Garmin Connect), plus assistance, batterie et autonomie
...
Les sorties de build (`bin/`, `*.iq`, `*.prg`, `*.debug.xml`) et les clés de développement (`*.pem`, `*.der`) sont exclues par `.gitignore`.
...
**[English](README.md)** — English version of the README.